...
-- opens_at / closes_at 생성 열은 두지 않는다.
-- timestamptz +/- interval 은 STABLE이라 생성 열 표현식으로 쓸 수 없다(42P17).
-- 창 경계 판정은 lib/window.ts(T08)가 순수 함수로 수행하고,
-- 쿼리에서 필요하면 where 절에서 직접 계산한다.
