...
describe('distanceM', () => {
  ...
  // 아래 두 케이스의 기댓값은 하버사인 산출값을 pin 한 게 아니라 해석적으로 유도한
  // 외부 기준값이다. 지구 반지름 상수나 도→라디안 변환이 틀어지면 바로 어긋난다.

  // 경도가 같으므로 대원 호의 중심각이 정확히 1도다.
  // 호 길이 = π · 6371000 / 180 = 111194.9266445587m
  it('자오선 1도가 해석적 호 길이와 일치한다', () => {
    const d = distanceM({ lat: 37, lng: 127 }, { lat: 38, lng: 127 })
    expect(d).toBeCloseTo(111194.9266445587, 6)
  })

  // 같은 자오선 위 0.001도 = 위 호 길이의 1/1000 = 111.1949266445587m.
  // 체크인 반경 스케일에서 밀리미터까지 맞는지를 여기서 본다 — 268km 케이스의
  // ±1km 허용오차는 이 스케일로 환산하면 37cm라 근거리 오차를 못 잡는다.
  it('체크인 반경 스케일에서 밀리미터까지 맞는다', () => {
    const d = distanceM({ lat: 37, lng: 127 }, { lat: 37.001, lng: 127 })
    expect(d).toBeCloseTo(111.1949266445587, 6)
  })
})
